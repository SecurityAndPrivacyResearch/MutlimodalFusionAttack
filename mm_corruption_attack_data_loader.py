...
class VqaDataset(data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    orig_questions = []
    question_ids = []
    #for i, j in zip(vqa[idx]['questions'], vqa[idx]['question_tokens']):
    for i in vqa[idx]['questions']:
      question_id = i['question_id']
      question_ids.append(question_id)
      qst2idc = np.array([qst_vocab.word2idx('<pad>')] * max_qst_length)  # padded with '<pad>' in 'ans_vocab'
      qst2idc[:len(i['question_tokens'])] = [qst_vocab.word2idx(w) for w in i['question_tokens']]
      questions.append(qst2idc)
      orig_questions.append(i['question_tokens'])

    sample = {'image': image, 'questions': questions, 'orig_questions': orig_questions, 'image_path': image_path, 'question_ids': question_ids, 'image_name': image_name}

    if load_ans:
      answers = []
      answers_multi_choice = []
      #for i in zip(vqa[idx]['questions']):
      for i in vqa[idx]['questions']:
        ans2idc = [ans_vocab.word2idx(w) for w in i['valid_answers']]
        ans2idx = np.random.choice(ans2idc)
        answers.append(ans2idx)
        ...
        answers_multi_choice.append(mul2idc)
        #sample['answer_multi_choice'] = mul2idc  # for evaluation metric of 'multiple choice'

      sample['answers_label'] = answers  
      sample['answers_multi_choice'] = answers_multi_choice
